...
- 🎯 Entry points to execute your program using `python -m <dms_epicor>` or `$ dms_epicor` with basic CLI argument parsing.
...
---
# dms_epicor Flask Application

Awesome dms_epicor created by thanvanhai
...
```bash
git clone https://github.com/thanvanhai/DMS_Epicor dms_epicor
cd dms_epicor
make install
```
...
```bash
dms_epicor create-db   # run once
dms_epicor populate-db  # run once (optional)
dms_epicor add-user -u admin -p 1234  # ads a user
dms_epicor run
```
