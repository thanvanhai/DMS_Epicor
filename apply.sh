...
original_author="thanvanhai"
original_name="dms_epicor"
original_urlname="DMS_Epicor"
original_description="Awesome dms_epicor created by thanvanhai" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/dms_epicor "${name}"
...
then
    ...
    echo "then run 'dms_epicor' or 'python -m dms_epicor'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'dms_epicor' or 'python -m dms_epicor' or 'poetry run dms_epicor'"
fi
